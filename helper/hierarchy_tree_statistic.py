...
class DatasetStatistic(object):
    def __init__(self, config):
        ...
        self.hierarchical_label_dict, self.label_vocab = self.get_hierar_relations_with_name(
            'E:\\LibSpace\\HiFlaky\\data\\flaky_label.taxonomy')
        self.level = 0
    ...
    def get_taxonomy_file(self):
        ...
        print(f"hierarcy_dict:{hierarcy_dict}")
        known_label = ['Root']
        output_lines = []
...
if __name__ == '__main__':
    config = Configure(config_json_file="E:\\LibSpace\\HiFlaky\\config\\test_config.json")

    flaky_dataset_statistic = DatasetStatistic(config)

    train_statistics = flaky_dataset_statistic.get_data_statistic(
        "E:\\LibSpace\\HiFlaky\\data\\flaky_token_train.json")
    val_statistics = flaky_dataset_statistic.get_data_statistic(
        "E:\\LibSpace\\HiFlaky\\data\\flaky_token_val.json")
    test_statistics = flaky_dataset_statistic.get_data_statistic(
        "E:\\LibSpace\\HiFlaky\\data\\flaky_token_test.json")
    ...
    print('*****TOTAL*****')
    print(flaky_dataset_statistic.prior_prob_dict)
    print(prior_probs(flaky_dataset_statistic.prior_prob_dict))
    print('*****TOTAL TRAIN*****')
    print(flaky_dataset_statistic.total_train_prob_dict)
    print(prior_probs(flaky_dataset_statistic.total_train_prob_dict))
    train_probs = prior_probs(flaky_dataset_statistic.total_train_prob_dict)
    with open('E:\\LibSpace\\HiFlaky\\data\\flaky_prob.json', 'w') as json_file:
